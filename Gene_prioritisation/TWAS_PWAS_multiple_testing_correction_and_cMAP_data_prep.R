...
write.table(Colocalised_PWAS_TWAS, file="Updated_for_Zac/Colocalised_H4_0.8_TWAS_PWAS.txt",
            sep = "\t", row.names = F, quote = F)
...
write.table(Colocalised_PWAS_TWAS_RBP4, file="Updated_for_Zac/Colocalised_H4_0.8_TWAS_PWAS_pQTL_MR_RBP4.txt",
            sep = "\t", row.names = F, quote = F)

## Suggestive coloc

Sug_TWAS_coloc <- Sug_coloc_transcriptome_wide %>% select(gene_name, TWAS.Z, TWAS.P, COLOC.PP4)
Sug_TWAS_coloc <- rename(Sug_TWAS_coloc, "ID"="gene_name", "Z"="TWAS.Z", "P"="TWAS.P")
Sug_PWAS_coloc <- Sug_coloc_proteome_wide %>% select(ID, PWAS.Z, PWAS.P, COLOC.PP4)
Sug_PWAS_coloc <- rename(Sug_PWAS_coloc, "Z"="PWAS.Z", "P"="PWAS.P")

Sug_Colocalised_PWAS_TWAS <- rbind(Sug_TWAS_coloc, Sug_PWAS_coloc)

write.table(Sug_Colocalised_PWAS_TWAS, file="Updated_for_Zac/Colocalised_H4_0.4_TWAS_PWAS.txt",
            sep = "\t", row.names = F, quote = F)
