...
Full <- fread("~/data/users/william/Meta_retinol_GWAS/vitd.gwama.out.gz")
Full$SNP <- Full$rs_number
...
Sorted_df <- Sorted_df %>% filter(A1 != reference_allele | A1 != other_allele)
...
## 0.1500895 proportion of SNPs with non-null effects (both causal and tagging SNPs)
...
## Check if any mismatches
Sorted_df_2 <- Sorted_df_2 %>% filter(A1 != reference_allele | A1 != other_allele)
