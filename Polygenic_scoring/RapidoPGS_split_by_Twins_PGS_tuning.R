...
## RapidoPGS - Retinol PGS tuning and association
...
PGS_merge_subset_2 <- merge(Merge_Rapido, Merged_ret_PCs_Subset_2, by = "PublicID")
...
Scores_to_test <- as.list(colnames(PGS_merge_subset_2 %>% dplyr::select(starts_with("Default") | starts_with("Inferred"))))
...
write.table(Three_visit_concat, file="../RapidoPGS_approach/RapidoPGS_split_twins_Tuning_subset_r2_per_visit.txt",
...
PGS_merge_Subset_1 <- merge(Merge_Rapido, Merged_ret_PCs_Subset_1, by = "PublicID")
...
write.table(Val_Three_visit_concat, file="../RapidoPGS_approach/RapidoPGS_split_twins_Validation_subset_r2_per_visit.txt",
            sep = "\t", row.names = F, quote = F)
...
ggplot(Val_Plot_melt, aes(x = Score, y = value, colour = as.factor(Score), group = 1)) +
...
Linear_1 <- ggplot(PGS_merge_Subset_1, aes(x=scale(Inferred_prior_SD_PGS), y=scale(Retinol_visit1))) +
  geom_point(cex = 1) +
...
Linear_2 <- ggplot(PGS_merge_Subset_1, aes(x=scale(Inferred_prior_SD_PGS), y=scale(Retinol_visit2))) +
  geom_point(cex = 1) +
  stat_smooth(method = "lm", formula = y ~ x, size = 1) +
  theme_bw() +
  xlab("Retinol PGS (SD)") +
...
Linear_3 <- ggplot(PGS_merge_Subset_1, aes(x=scale(Inferred_prior_SD_PGS), y=scale(Retinol_visit3))) +
  geom_point(cex = 1) +
  stat_smooth(method = "lm", formula = y ~ x, size = 1) +
  theme_bw() +
  xlab("Retinol PGS (SD)") +
...
PGS_merge_Subset_1$PGS_quint <- ntile(PGS_merge_Subset_1$Inferred_prior_SD_PGS, 5)

Q1 <- ggplot(data = PGS_merge_Subset_1, aes(x=as.factor(PGS_quint), y = scale(Retinol_visit1), fill = as.factor(PGS_quint))) +
...
Q2 <- ggplot(data = PGS_merge_Subset_1, aes(x=as.factor(PGS_quint), y = scale(Retinol_visit2), fill = as.factor(PGS_quint))) +
  geom_boxplot(width=0.4) +
  geom_hline(yintercept = 0, linetype = "dashed", colour = "red") +
  theme_bw() +
  theme(legend.position = "none") +
...
Q3 <- ggplot(data = PGS_merge_Subset_1, aes(x=as.factor(PGS_quint), y = scale(Retinol_visit3), fill = as.factor(PGS_quint))) +
  geom_boxplot(width=0.4) +
  geom_hline(yintercept = 0, linetype = "dashed", colour = "red") +
  theme_bw() +
  theme(legend.position = "none") +
...
## Test effect of quintile 5 vs quintile 3 at visit 2

Quintile_df <- PGS_merge_Subset_1 %>% filter(PGS_quint == 3 | PGS_quint == 5)

Quintile_reg <- lm(scale(Retinol_visit2) ~ Age_visit2 + Batch_visit2 +
                   PC1 + PC2 + PC3 + PC4 + PC5 + PGS_quint,
                   data = Quintile_df)

## Ret_quint_2 - 5 vs 3     0.8617046  0.0327054  26.347   <2e-16 ***

## Read in PGS weights

PGS_weights <- fread("../RapidoPGS_approach/PP_weighted_PGS_inferred_SD_prior_IVW.txt")

PGS_weights %>% filter(abs(PP_Beta) > 0.01)

## Roughly equivalent to genome-wide sig SNP score
