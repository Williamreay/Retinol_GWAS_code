...
rm(list = ls())
setwd("~/Desktop/Retinol_GWAS/TwinsUK_app/E1205_20.01.2023/Maria_normative_modelling/Within_subset_1_GAMLSS/") #$AMEND$
source('helpers.R')
...
set.seed(2838622)                                                                                                                                                         
#set out path $AMEND$
PATH_OUT <- paste0(getwd(), "/gamlss_output_visit_3/")

# read data currently modeling baseline 
df <- read.table("../../Subset_1_females_half_of_twin_pairs_TwinsUK.txt", sep = "\t", header = TRUE)

## Remove any missing values - retinol visit 1 as a test
df <- df %>% filter(!is.na(Retinol_visit3)) %>% filter(!is.na(BMI_visit3))


mydata_tmp <- df %>% dplyr::select(Age_visit3, Batch_visit3, BMI_visit3, Retinol_visit3, Last_digit, PublicID)
...
#select half of the subjects for model training (STEP 2) 

n <- length(mydata_tmp$age)
split_idx <- sample(1:n, size = n/2, replace = FALSE)
idx1 <- split_idx
idx2 <- setdiff(1:n, split_idx)
...
## Without BMI fits marginally better delta AIC = 6.378385
...
TE <- readMat("gamlss_output_visit_3/GAMLSSout_INDV_DEVIATIONS_MODEL1.mat")

Ids <- mydata_tmp %>% dplyr::select(ID)

Dev_3 <- as.data.frame(cbind(Ids, band_normal, band_infra, band_supra))

Dev_3 <- dplyr::rename(Dev_3, "publicid"="ID")

write.table(Dev_3, file="Half_sample_Visit_3_deviations_BCT.txt",
            sep = "\t", row.names = F)
