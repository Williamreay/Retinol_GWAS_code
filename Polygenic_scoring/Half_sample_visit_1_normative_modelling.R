...
set.seed(2838622)                                                                                                                                                          
#set out path $AMEND$
PATH_OUT <- paste0(getwd(), "/gamlss_output/")
...
df <- df %>% filter(!is.na(Retinol_visit1))


mydata_tmp <- df %>% dplyr::select(Age_visit1, Batch_visit1, BMI_visit1, Retinol_visit1, Last_digit, PublicID)
...
## Without BMI fits marginally better delta AIC = 1.622361
...
TE <- readMat("gamlss_output/GAMLSSout_INDV_DEVIATIONS_MODEL1.mat")

Ids <- mydata_tmp %>% dplyr::select(ID)

Dev_1 <- as.data.frame(cbind(Ids, band_normal, band_infra, band_supra))

Dev_1 <- dplyr::rename(Dev_1, "publicid"="ID")

write.table(Dev_1, file="Half_sample_Visit_1_deviations_SHASH.txt",
            sep = "\t", row.names = F)
