...
mydata_tmp <- df %>% dplyr::select(Age_visit2, Batch_visit2, BMI_visit2, Retinol_visit2, Last_digit, PublicID)
...
Dev_2 <- as.data.frame(cbind(Ids, band_normal, band_infra, band_supra))

Dev_2 <- dplyr::rename(Dev_2, "publicid"="ID")

write.table(Dev_2, file="Half_sample_Visit_2_deviations_BCT.txt",
            sep = "\t", row.names = F)
